CREATE OR REPLACE PROCEDURE ds.getproductpropensitytrainingdata(_lkupclientid int4, season_min int4, season_max int4, rs_out refcursor)
	LANGUAGE plpgsql
AS $$
...
BEGIN
	...
	where seasonYear <> 2020
	and cast(cp.seasonYear as int) between season_min and season_max -1
	...
	where 
		...
		and cast(cp.seasonYear as int) between season_min and season_max -2
	...
	where
		sbc_now.seasonYear between season_min_train and season_max -1
	group by
		sbc_now.dimCustomerMasterId,
		sbc_now.seasonYear
	having
